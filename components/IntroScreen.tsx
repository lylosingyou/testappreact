...
const IntroScreen: React.FC<IntroScreenProps> = ({ onComplete }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center bg-black text-[#F8F8F8] p-4 md:p-8 overflow-y-auto no-scrollbar">
        <div className="max-w-3xl w-full flex flex-col space-y-6 text-center my-auto py-8">
            ...
            <h3 className={`text-lg md:text-xl font-bold mt-8 transition-opacity duration-1000 ${visibleElements > introParagraphs.length ? 'opacity-100' : 'opacity-0'}`}>
                Core Principles:
            </h3>
            ...
            <div className={`pt-8 transition-opacity duration-1000 ${visibleElements > introParagraphs.length + principles.length + 1 ? 'opacity-100' : 'opacity-0'}`}>
                <button
                    onClick={onComplete}
                    className="px-8 md:px-12 py-3 md:py-4 border-2 border-gray-400/50 text-gray-200 bg-gray-500/10 rounded-sm text-lg md:text-2xl tracking-widest transition-all duration-300 hover:bg-gray-500/30 hover:border-gray-300/80 hover:shadow-[0_0_25px_rgba(255,255,255,0.3)]"
                >
                    I am human
                </button>
            </div>
